...
- CUDA graph capture for the no-loss backward region, not the full train step.
...
- The proposal's full quantization proc-macro compiler was not implemented; the current macro is a constant-layout declaration with audit gates.
- BigramHash forward merge existed, but fused backward had only just landed and still needed H100 parity/perf evidence.
...
| --- | ---: | ---: | ---: |
| Exact recurrent boundary fusion | 137.73 | 148.85 | 120.96 |
...
- Full quantization procedural-macro compiler.
