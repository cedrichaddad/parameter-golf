...
mod tests {
    ...
    fn frontier_2014_and_2135_specs_load_runtime_targets() {
        ...
        assert_eq!(
            exact_artifact_eval_compat.eval.ttt_lora_targets,
            audit.eval.ttt_lora_targets
        );
        assert!(
            exact_artifact_eval_compat.allow_unsupported_variants,
            "eval-only compatibility spec is diagnostic and must not masquerade as a clean record spec"
        );
        ExecutionPlan::from_run_spec(&exact_artifact_eval_compat).unwrap();
