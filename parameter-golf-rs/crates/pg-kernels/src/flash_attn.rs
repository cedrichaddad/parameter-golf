...
unsafe extern "C" {
    ...
    fn run_cudnn_sdpa_bf16_f32_backward_with_saved_bf16_stats_bf16_grads(
        stream: *mut c_void,
        q_bf16: u64,
        k_bf16: u64,
        v_bf16: u64,
    ...
    fn run_cudnn_sdpa_bf16_backward_with_saved_bf16_stats_bhsd_do_bf16_grads(
        stream: *mut c_void,
        q_bf16: u64,
        k_bf16: u64,
        v_bf16: u64,
...
impl CudnnFrontendAttention {
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn backward_with_saved_bf16_stats_bf16_grads(
        &self,
        q_bf16: u64,
        k_bf16: u64,
        v_bf16: u64,
        ...
        #[cfg(has_cudnn_frontend_sdpa)]
        unsafe {
            let status = run_cudnn_sdpa_bf16_f32_backward_with_saved_bf16_stats_bf16_grads(
                self.stream.cu_stream() as *mut c_void,
                q_bf16,
                k_bf16,
                v_bf16,
            ...
            if status != 0 {
                return Err(PgError::InvalidOp(format!(
                    "cuDNN frontend SDPA backward_with_saved_bf16_stats_bf16_grads failed with status code {}",
                    status
                )));
            }
            Ok(())
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn backward_with_saved_bf16_stats_bhsd_do_bf16_grads(
        &self,
        q_bf16: u64,
        k_bf16: u64,
        v_bf16: u64,
        ...
        #[cfg(has_cudnn_frontend_sdpa)]
        unsafe {
            let status = run_cudnn_sdpa_bf16_backward_with_saved_bf16_stats_bhsd_do_bf16_grads(
                self.stream.cu_stream() as *mut c_void,
                q_bf16,
                k_bf16,
                v_bf16,
            ...
            if status != 0 {
                return Err(PgError::InvalidOp(format!(
                    "cuDNN frontend SDPA backward_with_saved_bf16_stats_bhsd_do_bf16_grads failed with status code {}",
                    status
                )));
            }
            Ok(())
