...
pub struct GpuKernels {
    ...
    rms_norm_bwd_residual_mix_go_bf16_add_chunked: CudaFunction,
    rms_norm_bwd_residual_mix_go_bf16_add_chunked_compact: CudaFunction,
    ...
    leaky_relu_sq_fwd_x_bf16_only_vec4: CudaFunction,
    ...
    leaky_relu_sq_bwd_x_bf16_only_fast: CudaFunction,
    leaky_relu_sq_bwd_x_bf16_only_vec2: CudaFunction,
    leaky_relu_sq_bwd_x_bf16_only_vec4_fast: CudaFunction,
    leaky_relu_sq_bwd_x_go_bf16_only: CudaFunction,
    leaky_relu_sq_bwd_x_go_bf16_only_fast: CudaFunction,
    leaky_relu_sq_bwd_x_go_bf16_only_vec4_fast: CudaFunction,
    ...
    residual_add_scale_bwd_assign: CudaFunction,
    ...
    residual_add_scale_bwd_bf16_only_no_atomic_add: CudaFunction,
    residual_add_scale_grad_scale_reduce: CudaFunction,
    residual_add_scale_grad_scale_reduce_bf16: CudaFunction,
    residual_add_scale_bwd_from_bf16_only_chunked_stage1: CudaFunction,
    residual_add_scale_bwd_bf16_only_chunked_stage1: CudaFunction,
    residual_add_scale_bwd_from_bf16_only_chunked_stage1_add: CudaFunction,
    ...
    q_gain_rope_qk_norm_bwd_chunked_go_bf16_pack_q: CudaFunction,
    qkv_rope_qk_norm_bwd_chunked_go_bf16_pack: CudaFunction,
    ...
    rope_qk_norm_bwd_go_bf16_pack_kv: CudaFunction,
    dot_accumulate: CudaFunction,
    scale_square_accumulate: CudaFunction,
    bf16_to_f32_scale_square_accumulate: CudaFunction,
    ...
    nesterov_update_hyper: CudaFunction,
    scale_add_inplace_hyper_momentum: CudaFunction,
    scale_add_inplace_global_norm_hyper_momentum: CudaFunction,
    ...
    sparse_attn_gate_xsa_fwd_bf16_bhsd_warpheads: CudaFunction,
    ...
    synthetic_record_batch_u32: CudaFunction,
    ...
    unpack_qkv_output_save_qk: CudaFunction,
    unpack_qkv_rope_gain_prepack_fwd: CudaFunction,
    unpack_qkv_rope_gain_prepack_fwd_bf16_only: CudaFunction,
    ...
    decay_sgd_step_bf16_shadow: CudaFunction,
    decay_sgd_step_hyper: CudaFunction,
...
__device__ __forceinline__ float pg_softcap_asym(float value, float softcap_pos, float softcap_neg) {
    float cap = (value >= 0.0f) ? softcap_pos : softcap_neg;
    if (cap <= 0.0f) return value;
    return cap * tanhf(value / cap);
}

__device__ __forceinline__ float pg_softcap_asym_deriv(float value, float softcap_pos, float softcap_neg) {
    float cap = (value >= 0.0f) ? softcap_pos : softcap_neg;
    if (cap <= 0.0f) return 1.0f;
    float tv = tanhf(value / cap);
    return 1.0f - tv * tv;
}
...
extern "C" __global__ void synthetic_record_batch_u32(
    unsigned int* __restrict__ input_ids,
    unsigned int* __restrict__ targets,
    unsigned long long step,
    unsigned long long global_batch_tokens,
    unsigned int rank,
    unsigned int world_size,
    unsigned int seq_len,
    unsigned int local_microbatches,
    unsigned int vocab_size
) {
    unsigned long long local_tokens =
        static_cast<unsigned long long>(seq_len) * local_microbatches;
    unsigned long long idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= local_tokens) return;

    unsigned long long micro_idx = idx / seq_len;
    unsigned long long pos = idx - micro_idx * seq_len;
    unsigned long long offset = step * global_batch_tokens
        + micro_idx * static_cast<unsigned long long>(seq_len) * world_size
        + static_cast<unsigned long long>(rank) * seq_len;
    unsigned int input = static_cast<unsigned int>((offset + pos) % vocab_size);
    unsigned int target = static_cast<unsigned int>((offset + pos + 1ull) % vocab_size);
    input_ids[idx] = input;
    targets[idx] = target;
}
...
// combined[t, d+2kv] -> q[t,d], k[t,kv], v[t,kv], with q/k also saved
// for QK-norm/RoPE backward. This removes the extra q/k copy pass in the
// saved-activation training path.
extern "C" __global__ void unpack_qkv_output_save_qk(
    const float* __restrict__ combined,
    float* __restrict__ q,
    float* __restrict__ k,
    float* __restrict__ v,
    float* __restrict__ q_save,
    float* __restrict__ k_save,
    int tokens,
    int d,
    int kv
) {
    ...
    float value = combined[idx];
    if (col < d) {
        int out_idx = row * d + col;
        q[out_idx] = value;
        q_save[out_idx] = value;
    } else if (col < d + kv) {
        int out_idx = row * kv + (col - d);
        k[out_idx] = value;
        k_save[out_idx] = value;
    } else {
        v[row * kv + (col - d - kv)] = value;
    }
}

extern "C" __global__ void unpack_qkv_rope_gain_prepack_forward(
    const float* __restrict__ combined,
    float* __restrict__ q,
    float* __restrict__ k,
    float* __restrict__ v,
    ...
    float q_sum = 0.0f;
    float k_sum = 0.0f;
    bool has_q = head < num_heads;
    bool has_kv = head < num_kv_heads;
    int q_base_combined = tok * qkv + head * head_dim;
    int k_base_combined = tok * qkv + d + head * head_dim;
    int v_base_combined = tok * qkv + d + kv + head * head_dim;
    int q_base = tok * d + head * head_dim;
    int k_base = tok * kv + head * head_dim;

    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        if (has_q) {
            float qv = combined[q_base_combined + j];
            q_sum += qv * qv;
        }
        if (has_kv) {
            float kvv = combined[k_base_combined + j];
            k_sum += kvv * kvv;
        }
    }
    ...
    __shared__ float shared_k[32];
    int lane = threadIdx.x % 32;
    int warp_id = threadIdx.x / 32;
    if (lane == 0) {
        shared_q[warp_id] = q_sum;
        shared_k[warp_id] = k_sum;
    }
    __syncthreads();

    if (threadIdx.x < 32) {
        int warps = (blockDim.x + 31) / 32;
        q_sum = (threadIdx.x < warps) ? shared_q[threadIdx.x] : 0.0f;
        k_sum = (threadIdx.x < warps) ? shared_k[threadIdx.x] : 0.0f;
    ...
    __shared__ float q_inv_rms_shared;
    __shared__ float k_inv_rms_shared;
    if (threadIdx.x == 0) {
        q_inv_rms_shared = has_q ? rsqrtf(q_sum / (float)head_dim + eps) : 0.0f;
        k_inv_rms_shared = has_kv ? rsqrtf(k_sum / (float)head_dim + eps) : 0.0f;
    }
    __syncthreads();
    float q_inv_rms = q_inv_rms_shared;
    float k_inv_rms = k_inv_rms_shared;

    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        if (has_q) {
            float q_raw = combined[q_base_combined + j];
            q_save[q_base + j] = q_raw;
        }
        if (has_kv) {
            float k_raw = combined[k_base_combined + j];
            float v_raw = combined[v_base_combined + j];
            k_save[k_base + j] = k_raw;
            v[k_base + j] = v_raw;
            int v_out_idx = ((b * num_kv_heads + head) * seq_len + pos) * head_dim + j;
            v_bhsd_bf16[v_out_idx] = pg_f32_to_bf16(v_raw);
        }
    }
    ...
    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        if (has_q) {
            ...
            if (j < half) {
                ...
                float x1 = combined[q_base_combined + j] * q_inv_rms;
                float x2 = combined[q_base_combined + half + j] * q_inv_rms;
                q_post = x1 * cos_val + x2 * sin_val;
                float q_peer_post = -x1 * sin_val + x2 * cos_val;
                q_out = q_post * g;
                float q_peer_out = q_peer_post * g;
                q[q_base + j] = q_out;
                q[q_base + half + j] = q_peer_out;
                q_post_rope_save[q_base + j] = q_post;
                q_post_rope_save[q_base + half + j] = q_peer_post;
                int q_out_idx = ((b * num_heads + head) * seq_len + pos) * head_dim;
                q_bhsd_bf16[q_out_idx + j] = pg_f32_to_bf16(q_out);
                q_bhsd_bf16[q_out_idx + half + j] = pg_f32_to_bf16(q_peer_out);
            } else if (j >= rope_dims) {
                q_post = combined[q_base_combined + j] * q_inv_rms;
                q_out = q_post * g;
                q[q_base + j] = q_out;
                q_post_rope_save[q_base + j] = q_post;
                int q_out_idx = ((b * num_heads + head) * seq_len + pos) * head_dim + j;
                q_bhsd_bf16[q_out_idx] = pg_f32_to_bf16(q_out);
            }
        }
        ...
        if (has_kv) {
            ...
            if (j < half) {
                float cos_val = cos_table[pos * half + j];
                float sin_val = sin_table[pos * half + j];
                float x1 = combined[k_base_combined + j] * k_inv_rms;
                float x2 = combined[k_base_combined + half + j] * k_inv_rms;
                k_out = x1 * cos_val + x2 * sin_val;
                float k_peer_out = -x1 * sin_val + x2 * cos_val;
                k[k_base + j] = k_out;
                k[k_base + half + j] = k_peer_out;
                int k_out_idx = ((b * num_kv_heads + head) * seq_len + pos) * head_dim;
                k_bhsd_bf16[k_out_idx + j] = pg_f32_to_bf16(k_out);
                k_bhsd_bf16[k_out_idx + half + j] = pg_f32_to_bf16(k_peer_out);
            } else if (j >= rope_dims) {
                k_out = combined[k_base_combined + j] * k_inv_rms;
                k[k_base + j] = k_out;
                int k_out_idx = ((b * num_kv_heads + head) * seq_len + pos) * head_dim + j;
                k_bhsd_bf16[k_out_idx] = pg_f32_to_bf16(k_out);
            }
        }
    }
}

// Forward-only variant for recurrent pass-1 straight-through. It preserves the
// fast cuDNN prepacked BF16 attention path, but skips all F32 q/k/v and
// backward-save writes that the pass-1 ST backward never consumes.
extern "C" __global__ void unpack_qkv_rope_gain_prepack_forward_bf16_only(
    const float* __restrict__ combined,
    unsigned short* __restrict__ q_bhsd_bf16,
    unsigned short* __restrict__ k_bhsd_bf16,
    unsigned short* __restrict__ v_bhsd_bf16,
    ...
    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        if (has_q) {
            ...
            if (j < half) {
                ...
                float x1 = combined[q_base_combined + j] * q_inv_rms;
                float x2 = combined[q_base_combined + half + j] * q_inv_rms;
                float q_post = x1 * cos_val + x2 * sin_val;
                float q_peer_post = -x1 * sin_val + x2 * cos_val;
                int q_out_idx = ((b * num_heads + head) * seq_len + pos) * head_dim;
                q_bhsd_bf16[q_out_idx + j] = pg_f32_to_bf16(q_post * g);
                q_bhsd_bf16[q_out_idx + half + j] = pg_f32_to_bf16(q_peer_post * g);
            } else if (j >= rope_dims) {
                float q_out = combined[q_base_combined + j] * q_inv_rms * g;
                int q_out_idx = ((b * num_heads + head) * seq_len + pos) * head_dim + j;
                q_bhsd_bf16[q_out_idx] = pg_f32_to_bf16(q_out);
            }
        }
        ...
        if (has_kv) {
            if (j < half) {
                ...
                float x1 = combined[k_base_combined + j] * k_inv_rms;
                float x2 = combined[k_base_combined + half + j] * k_inv_rms;
                float k_out = x1 * cos_val + x2 * sin_val;
                float k_peer_out = -x1 * sin_val + x2 * cos_val;
                int k_out_idx = ((b * num_kv_heads + head) * seq_len + pos) * head_dim;
                k_bhsd_bf16[k_out_idx + j] = pg_f32_to_bf16(k_out);
                k_bhsd_bf16[k_out_idx + half + j] = pg_f32_to_bf16(k_peer_out);
            } else if (j >= rope_dims) {
                float k_out = combined[k_base_combined + j] * k_inv_rms;
                int k_out_idx = ((b * num_kv_heads + head) * seq_len + pos) * head_dim + j;
                k_bhsd_bf16[k_out_idx] = pg_f32_to_bf16(k_out);
            }
        }
...
extern "C" __global__ void decay_sgd_step_bf16_shadow(
    float* __restrict__ param,
    unsigned short* __restrict__ param_bf16,
    const float* __restrict__ grad,
    float lr,
    float weight_decay,
    int n
    ...
    if (idx >= n) return;
    float decay = 1.0f - lr * weight_decay;
    float p = decay * param[idx] - lr * grad[idx];
    param[idx] = p;
    param_bf16[idx] = pg_f32_to_bf16(p);
}

// hyper layout: [lr, momentum, weight_decay, reserved].
extern "C" __global__ void decay_sgd_step_hyper(
    float* __restrict__ param,
    const float* __restrict__ grad,
    ...
    if (idx >= n) return;
    float lr = hyper[0];
    float weight_decay = hyper[2];
    float decay = 1.0f - lr * weight_decay;
    param[idx] = decay * param[idx] - lr * grad[idx];
}

extern "C" __global__ void decay_sgd_step_bf16_shadow_hyper(
    float* __restrict__ param,
    unsigned short* __restrict__ param_bf16,
    const float* __restrict__ grad,
    const float* __restrict__ hyper,
...
extern "C" __global__ void adamw_step_bf16_shadow(
    float* __restrict__ param,
    unsigned short* __restrict__ param_bf16,
    const float* __restrict__ grad,
    float* __restrict__ m,
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_add_compact(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_add_no_mix_grad(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra,
    const float* __restrict__ residual_x,
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_add_compact_no_mix_grad(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra_compact,
    const float* __restrict__ residual_x,
...
extern "C" __global__ void residual_mix_grad_reduce_from_stats_go_bf16_add(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra,
    const float* __restrict__ residual_x,
    ...
    for (int row = row_start + threadIdx.x; row < row_end; row += blockDim.x) {
        ...
        float go_norm = pg_bf16_to_f32(grad_norm_bf16[idx]) + grad_norm_extra[idx];
        float norm_dx = inv_rms * (go_norm - x_norm[idx] * coeff);
        float go = beta * base_grad[idx] + norm_dx;
...
extern "C" __global__ void residual_mix_grad_reduce_from_stats_go_bf16_add_compact(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra_compact,
    const float* __restrict__ residual_x,
    ...
    for (int row = row_start + threadIdx.x; row < row_end; row += blockDim.x) {
        ...
        float coeff = row_norm_stats[row * 2 + 1];
        const float* extra_row = grad_norm_extra_compact + row * extra_width;
        float extra = (feature < extra_width) ? extra_row[feature] : 0.0f;
        float go_norm = pg_bf16_to_f32(grad_norm_bf16[idx]) + extra;
        float norm_dx = inv_rms * (go_norm - x_norm[idx] * coeff);
        float go = beta * base_grad[idx] + norm_dx;
        float residual = use_x0 ? residual_x0[idx] : residual_x[idx];
        sum += go * residual;
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_chunked(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ residual_x,
    const float* __restrict__ residual_x0,
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_add_chunked(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra,
    const float* __restrict__ residual_x,
...
extern "C" __global__ void rms_norm_backward_accum_residual_mix_backward_go_bf16_add_chunked_compact(
    const float* __restrict__ x_norm,
    const unsigned short* __restrict__ grad_norm_bf16,
    const float* __restrict__ grad_norm_extra_compact,
    const float* __restrict__ residual_x,
...
extern "C" __global__ void leaky_relu_sq_forward_x_bf16_only_vec4(
    const unsigned short* __restrict__ x_bf16,
    unsigned short* __restrict__ y_bf16,
    int n
) {
    ...
    int idx = group * 4;
    if (idx + 3 < n) {
        const unsigned long long packed_x =
            reinterpret_cast<const unsigned long long*>(x_bf16)[group];
        unsigned short x0_bf16 = static_cast<unsigned short>(packed_x & 0xffffull);
        unsigned short x1_bf16 = static_cast<unsigned short>((packed_x >> 16) & 0xffffull);
        unsigned short x2_bf16 = static_cast<unsigned short>((packed_x >> 32) & 0xffffull);
        unsigned short x3_bf16 = static_cast<unsigned short>((packed_x >> 48) & 0xffffull);

        float v0 = pg_bf16_to_f32(x0_bf16);
        float v1 = pg_bf16_to_f32(x1_bf16);
        float v2 = pg_bf16_to_f32(x2_bf16);
        float v3 = pg_bf16_to_f32(x3_bf16);
        float lr0 = (v0 >= 0.0f) ? v0 : 0.5f * v0;
        float lr1 = (v1 >= 0.0f) ? v1 : 0.5f * v1;
        float lr2 = (v2 >= 0.0f) ? v2 : 0.5f * v2;
        float lr3 = (v3 >= 0.0f) ? v3 : 0.5f * v3;

        unsigned long long packed_y =
            static_cast<unsigned long long>(pg_f32_to_bf16(lr0 * lr0)) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(lr1 * lr1)) << 16) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(lr2 * lr2)) << 32) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(lr3 * lr3)) << 48);
        reinterpret_cast<unsigned long long*>(y_bf16)[group] = packed_y;
        return;
    }

    for (int i = idx; i < n && i < idx + 4; ++i) {
        float v = pg_bf16_to_f32(x_bf16[i]);
        float lr = (v >= 0.0f) ? v : 0.5f * v;
        y_bf16[i] = pg_f32_to_bf16(lr * lr);
    }
}
...
extern "C" __global__ void leaky_relu_sq_backward_x_bf16_only_fast(
    const unsigned short* __restrict__ x_bf16,
    const float* __restrict__ grad_output,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    if (idx >= n) return;
    float v = pg_bf16_to_f32(x_bf16[idx]);
    float scale = (v >= 0.0f) ? 2.0f : 0.5f;
    grad_input_bf16[idx] = pg_f32_to_bf16(grad_output[idx] * v * scale);
}

extern "C" __global__ void leaky_relu_sq_backward_x_bf16_only_vec2(
    const unsigned short* __restrict__ x_bf16,
    const float* __restrict__ grad_output,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    int idx = pair_idx * 2;
    if (idx + 1 < n) {
        const unsigned int* x2 = reinterpret_cast<const unsigned int*>(x_bf16);
        const float2* go2 = reinterpret_cast<const float2*>(grad_output);
        unsigned int packed_x = x2[pair_idx];
        float2 go = go2[pair_idx];

        unsigned short x0_bf16 = static_cast<unsigned short>(packed_x & 0xffffu);
        unsigned short x1_bf16 = static_cast<unsigned short>(packed_x >> 16);
        float v0 = pg_bf16_to_f32(x0_bf16);
        float v1 = pg_bf16_to_f32(x1_bf16);

        float lr0 = (v0 >= 0.0f) ? v0 : 0.5f * v0;
        float lr1 = (v1 >= 0.0f) ? v1 : 0.5f * v1;
        float d_lr0 = (v0 >= 0.0f) ? 1.0f : 0.5f;
        float d_lr1 = (v1 >= 0.0f) ? 1.0f : 0.5f;
        unsigned short g0 = pg_f32_to_bf16(go.x * 2.0f * lr0 * d_lr0);
        unsigned short g1 = pg_f32_to_bf16(go.y * 2.0f * lr1 * d_lr1);

        unsigned int packed_grad =
            static_cast<unsigned int>(g0) | (static_cast<unsigned int>(g1) << 16);
        unsigned int* out2 = reinterpret_cast<unsigned int*>(grad_input_bf16);
        out2[pair_idx] = packed_grad;
    } else if (idx < n) {
        float v = pg_bf16_to_f32(x_bf16[idx]);
        float lr = (v >= 0.0f) ? v : 0.5f * v;
        float d_lr = (v >= 0.0f) ? 1.0f : 0.5f;
...
extern "C" __global__ void leaky_relu_sq_backward_x_bf16_only_vec4_fast(
    const unsigned short* __restrict__ x_bf16,
    const float* __restrict__ grad_output,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    if (idx + 3 < n) {
        const unsigned long long packed_x =
            reinterpret_cast<const unsigned long long*>(x_bf16)[group];
        const float4 go = reinterpret_cast<const float4*>(grad_output)[group];
        ...
        unsigned long long packed_grad =
            static_cast<unsigned long long>(pg_f32_to_bf16(go.x * v0 * ((v0 >= 0.0f) ? 2.0f : 0.5f))) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(go.y * v1 * ((v1 >= 0.0f) ? 2.0f : 0.5f))) << 16) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(go.z * v2 * ((v2 >= 0.0f) ? 2.0f : 0.5f))) << 32) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(go.w * v3 * ((v3 >= 0.0f) ? 2.0f : 0.5f))) << 48);
        reinterpret_cast<unsigned long long*>(grad_input_bf16)[group] = packed_grad;
        return;
    }
    ...
    for (int i = idx; i < n && i < idx + 4; ++i) {
        float v = pg_bf16_to_f32(x_bf16[i]);
        float scale = (v >= 0.0f) ? 2.0f : 0.5f;
        grad_input_bf16[i] = pg_f32_to_bf16(grad_output[i] * v * scale);
    }
}

extern "C" __global__ void leaky_relu_sq_backward_x_go_bf16_only(
    const unsigned short* __restrict__ x_bf16,
    const unsigned short* __restrict__ grad_output_bf16,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    float lr = (v >= 0.0f) ? v : 0.5f * v;
    float d_lr = (v >= 0.0f) ? 1.0f : 0.5f;
    float grad = pg_bf16_to_f32(grad_output_bf16[idx]) * 2.0f * lr * d_lr;
    grad_input_bf16[idx] = pg_f32_to_bf16(grad);
}

extern "C" __global__ void leaky_relu_sq_backward_x_go_bf16_only_fast(
    const unsigned short* __restrict__ x_bf16,
    const unsigned short* __restrict__ grad_output_bf16,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    float v = pg_bf16_to_f32(x_bf16[idx]);
    float scale = (v >= 0.0f) ? 2.0f : 0.5f;
    grad_input_bf16[idx] = pg_f32_to_bf16(pg_bf16_to_f32(grad_output_bf16[idx]) * v * scale);
}

extern "C" __global__ void leaky_relu_sq_backward_x_go_bf16_only_vec4_fast(
    const unsigned short* __restrict__ x_bf16,
    const unsigned short* __restrict__ grad_output_bf16,
    unsigned short* __restrict__ grad_input_bf16,
    int n
    ...
    if (idx + 3 < n) {
        const unsigned long long packed_x =
            reinterpret_cast<const unsigned long long*>(x_bf16)[group];
        const unsigned long long packed_go =
            reinterpret_cast<const unsigned long long*>(grad_output_bf16)[group];

        unsigned short x0_bf16 = static_cast<unsigned short>(packed_x & 0xffffull);
        unsigned short x1_bf16 = static_cast<unsigned short>((packed_x >> 16) & 0xffffull);
        unsigned short x2_bf16 = static_cast<unsigned short>((packed_x >> 32) & 0xffffull);
        unsigned short x3_bf16 = static_cast<unsigned short>((packed_x >> 48) & 0xffffull);
        unsigned short go0_bf16 = static_cast<unsigned short>(packed_go & 0xffffull);
        unsigned short go1_bf16 = static_cast<unsigned short>((packed_go >> 16) & 0xffffull);
        unsigned short go2_bf16 = static_cast<unsigned short>((packed_go >> 32) & 0xffffull);
        unsigned short go3_bf16 = static_cast<unsigned short>((packed_go >> 48) & 0xffffull);
        ...
        unsigned long long packed_grad =
            static_cast<unsigned long long>(pg_f32_to_bf16(pg_bf16_to_f32(go0_bf16) * v0 * ((v0 >= 0.0f) ? 2.0f : 0.5f))) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(pg_bf16_to_f32(go1_bf16) * v1 * ((v1 >= 0.0f) ? 2.0f : 0.5f))) << 16) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(pg_bf16_to_f32(go2_bf16) * v2 * ((v2 >= 0.0f) ? 2.0f : 0.5f))) << 32) |
            (static_cast<unsigned long long>(pg_f32_to_bf16(pg_bf16_to_f32(go3_bf16) * v3 * ((v3 >= 0.0f) ? 2.0f : 0.5f))) << 48);
        reinterpret_cast<unsigned long long*>(grad_input_bf16)[group] = packed_grad;
        return;
    }
    ...
    for (int i = idx; i < n && i < idx + 4; ++i) {
        ...
        float scale = (v >= 0.0f) ? 2.0f : 0.5f;
        grad_input_bf16[i] =
            pg_f32_to_bf16(pg_bf16_to_f32(grad_output_bf16[i]) * v * scale);
    }
}
...
extern "C" __global__ void residual_add_scale_backward_assign(
    const float* __restrict__ grad_output,
    const float* __restrict__ scale,
    float* __restrict__ grad_x_in,
    float* __restrict__ grad_proj,
...
extern "C" __global__ void residual_add_scale_backward_bf16_only_no_atomic_add(
    const float* __restrict__ grad_output,
    ...
    int d = idx % dim;
    float go = grad_output[idx];
    grad_x_in[idx] = go + add_input[idx];
    grad_proj_bf16[idx] = pg_f32_to_bf16(go * scale[d]);
}
...
// Coalesced two-stage residual scale backward. The older reducer assigns one
// block per feature dimension and walks rows with a `dim` stride. At record
// shape this is a large non-coalesced read. Stage 1 walks contiguous row-major
// elements, emits the BF16 projection gradient, copies the residual gradient,
// and reduces each row chunk into [chunk, dim] scratch. Stage 2 reduces that
// bounded scratch by dimension.
extern "C" __global__ void residual_add_scale_backward_from_bf16_only_chunked_stage1(
    const unsigned short* __restrict__ proj_bf16,
    const float* __restrict__ grad_output,
    const float* __restrict__ scale,
    float* __restrict__ grad_x_in,
    ...
    for (int linear = tid; linear < elems; linear += blockDim.x) {
        ...
        grad_x_in[idx] = go;
        grad_proj_bf16[idx] = pg_f32_to_bf16(go * scale[d]);
        atomicAdd(&sums[d], go * pg_bf16_to_f32(proj_bf16[idx]));
    }
    __syncthreads();
...
extern "C" __global__ void residual_add_scale_backward_from_bf16_only_chunked_stage1_add(
    const unsigned short* __restrict__ proj_bf16,
    const float* __restrict__ grad_output,
    const float* __restrict__ add_input,
    const float* __restrict__ scale,
...
extern "C" __global__ void residual_add_scale_backward_from_bf16_only_chunked_tiled_stage1(
    const unsigned short* __restrict__ proj_bf16,
    const float* __restrict__ grad_output,
    const float* __restrict__ scale,
    float* __restrict__ grad_x_in,
    ...
    int chunk = blockIdx.x;
    int tile = blockIdx.y;
    int dim_start = tile * dim_tile;
    if (dim_start >= dim) return;

    int tid = threadIdx.x;
    int lane_d = tid % dim_tile;
    int lane_r = tid / dim_tile;
    int row_lanes = blockDim.x / dim_tile;
    int d = dim_start + lane_d;
    bool valid_d = d < dim;
    ...
    if (valid_d) {
        float s = scale[d];
        for (int r = row_start + lane_r; r < row_end; r += row_lanes) {
            int idx = r * dim + d;
            float go = grad_output[idx];
            grad_x_in[idx] = go;
            grad_proj_bf16[idx] = pg_f32_to_bf16(go * s);
            sum += go * pg_bf16_to_f32(proj_bf16[idx]);
        }
    }
    scratch[tid] = sum;
    __syncthreads();

    for (int stride = row_lanes >> 1; stride > 0; stride >>= 1) {
        if (lane_r < stride && valid_d) {
            scratch[tid] += scratch[tid + stride * dim_tile];
        }
        __syncthreads();
    }

    if (lane_r == 0 && valid_d) {
        partial[chunk * dim + d] = scratch[tid];
    }
}

extern "C" __global__ void residual_add_scale_backward_bf16_only_chunked_tiled_stage1(
    const float* __restrict__ proj,
    const float* __restrict__ grad_output,
    const float* __restrict__ scale,
    float* __restrict__ grad_x_in,
...
extern "C" __global__ void residual_add_scale_backward_from_bf16_only_chunked_tiled_stage1_add(
    const unsigned short* __restrict__ proj_bf16,
    const float* __restrict__ grad_output,
    const float* __restrict__ add_input,
    const float* __restrict__ scale,
...
extern "C" __global__ void residual_add_scale_grad_scale_reduce_chunks(
...
// ──────────────────────────────────────────────────────────────
// Token embedding + BigramHash projection merge.
//
// out[t, j] = tok_emb[id[t], j]
//           + bigram_scale * dot(bigram_embed[hash(id[t-1], id[t])], bigram_proj[j, :])
//
// This is an opt-in record experiment for BigramHash-on specs. The current
// #2135 timing target keeps BigramHash disabled, so this kernel is isolated
// behind a runtime flag instead of perturbing the default forward graph.
// ──────────────────────────────────────────────────────────────
extern "C" __global__ void embedding_bigram_project_merge(
    const int* __restrict__ ids,
...
// Expand one compact shifted u16 span from a device-resident ring. Each ring
// slot has length tokens + 1 so target[t] can read the one-token lookahead
// without crossing into another slot.
extern "C" __global__ void shifted_u16_ring_to_u32(
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= tokens) return;
    const unsigned short* span = ring + ring_offset;
    input[idx] = (unsigned int)span[idx];
    target[idx] = (unsigned int)span[idx + 1];
}
...
extern "C" __global__ void q_gain_rope_qk_norm_backward_chunked_go_bf16_pack_q(
    const float* __restrict__ q_pre_norm,
    const float* __restrict__ q_post_rope,
    const unsigned short* __restrict__ grad_q_scaled_bf16,
    const float* __restrict__ gain,
    const float* __restrict__ cos_table,
    const float* __restrict__ sin_table,
    unsigned short* __restrict__ combined_qkv_bf16,
    float* __restrict__ grad_gain_chunks,
    int seq_len,
    int num_heads,
    int head_dim,
    ...
    const float* q_rope_head = q_post_rope + head_idx * head_dim;
    const unsigned short* go_scaled_head = grad_q_scaled_bf16 + head_idx * head_dim;
    unsigned short* q_dst = combined_qkv_bf16 + tok * qkv_dim + h * head_dim;
    float g = gain[h];
    ...
    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        ...
        float xv = x_head[j];
        q_dst[j] = pg_f32_to_bf16(inv_rms * (go - xv * coeff));
    }
}

extern "C" __global__ void qkv_rope_qk_norm_backward_chunked_go_bf16_pack(
    const float* __restrict__ q_pre_norm,
    const float* __restrict__ q_post_rope,
    const unsigned short* __restrict__ grad_q_scaled_bf16,
    ...
    int total_warps = num_heads + num_kv_heads;
    int block = blockIdx.x;
    int tok = block / total_warps;
    if (tok >= tokens) return;

    int head_group = block - tok * total_warps;
    ...
    if (head_group < num_heads) {
        int h = head_group;
        int head_idx = (tok * num_heads + h) * head_dim;
        const float* x_head = q_pre_norm + head_idx;
        const float* q_rope_head = q_post_rope + head_idx;
        const unsigned short* go_scaled_head = grad_q_scaled_bf16 + head_idx;
        unsigned short* q_dst = combined_qkv_bf16 + tok * qkv_dim + h * head_dim;
        float g = gain[h];
        ...
        for (int offset = 16; offset > 0; offset >>= 1) {
            ...
            x_dot_go += __shfl_down_sync(mask, x_dot_go, offset);
            gain_dot += __shfl_down_sync(mask, gain_dot, offset);
        }
        ...
        if (lane == 0) {
            float rms = sqrtf(sum_sq / (float)head_dim + eps);
            inv_rms = 1.0f / rms;
            coeff = x_dot_go / (rms * rms * (float)head_dim);
            int chunk = tok / q_gain_chunk_tokens;
            if (chunk >= q_gain_chunks) chunk = q_gain_chunks - 1;
            atomicAdd(&grad_gain_chunks[h * q_gain_chunks + chunk], gain_dot);
        }
        inv_rms = __shfl_sync(mask, inv_rms, 0);
        coeff = __shfl_sync(mask, coeff, 0);
    ...
    int h = head_group - num_heads;
    int head_idx = (tok * num_kv_heads + h) * head_dim;
    int kv_col_base = h * head_dim;
    const float* x_head = k_pre_norm + head_idx;
    const unsigned short* go_head = grad_k_attn_bf16 + head_idx;
    unsigned short* k_dst = combined_qkv_bf16 + tok * qkv_dim + d + kv_col_base;
    unsigned short* v_dst = combined_qkv_bf16 + tok * qkv_dim + d + kv + kv_col_base;
    const unsigned short* v_src = grad_v_bf16 + tok * kv + kv_col_base;
    const float* v_xsa_src = grad_v_xsa + tok * kv + kv_col_base;
    ...
    for (int j = lane; j < head_dim; j += 32) {
        ...
        float xv = x_head[j];
        k_dst[j] = pg_f32_to_bf16(inv_rms * (go - xv * coeff));
        float v = pg_bf16_to_f32(v_src[j]);
        if (add_v_xsa) v += v_xsa_src[j];
        v_dst[j] = pg_f32_to_bf16(v);
    }
}
...
extern "C" __global__ void rope_qk_norm_backward_go_bf16_pack_kv(
    const float* __restrict__ k_pre_norm,
    const unsigned short* __restrict__ grad_k_attn_bf16,
    const unsigned short* __restrict__ grad_v_bf16,
    const float* __restrict__ grad_v_xsa,
    ...
    for (int j = threadIdx.x; j < head_dim; j += blockDim.x) {
        ...
        if (add_v_xsa) {
            v_dst[j] = pg_f32_to_bf16(pg_bf16_to_f32(v_src[j]) + v_xsa_src[j]);
        } else {
            v_dst[j] = v_src[j];
        }
    }
...
// ──────────────────────────────────────────────────────────────
// Scale in-place and accumulate squared norm:
// x[i] *= scale; out[0] += alpha * sum_i(x[i]^2)
// ──────────────────────────────────────────────────────────────
extern "C" __global__ void scale_square_accumulate(
    float* __restrict__ x,
    float* __restrict__ out,
    float scale,
    float alpha,
...
// ──────────────────────────────────────────────────────────────
// Decode BF16, scale to F32, and accumulate squared norm:
// dst[i] = bf16_to_f32(src[i]) * scale;
// out[0] += alpha * sum_i(dst[i]^2)
// ──────────────────────────────────────────────────────────────
extern "C" __global__ void bf16_to_f32_scale_square_accumulate(
    const unsigned short* __restrict__ src,
    float* __restrict__ dst,
    float* __restrict__ out,
    float scale,
    ...
    float sum = 0.0f;
    for (int i = idx; i < n; i += blockDim.x * gridDim.x) {
        float v = pg_bf16_to_f32(src[i]) * scale;
        dst[i] = v;
        sum += v * v;
    }
...
extern "C" __global__ void cross_entropy_forward(
    ...
    for (int i = threadIdx.x; i < vocab_size; i += blockDim.x) {
        float capped = pg_softcap_asym(row[i], softcap, softcap_neg);
    ...
    if (threadIdx.x == 0) {
        ...
        float capped_target = pg_softcap_asym(row[target], softcap, softcap_neg);
...
extern "C" __global__ void cross_entropy_forward_bf16_logits(
    ...
    float max_val = -1e30f;
    for (int i = threadIdx.x; i < vocab_size; i += blockDim.x) {
        float capped = pg_softcap_asym(pg_bf16_to_f32(row[i]), softcap, softcap_neg);
        if (capped > max_val) max_val = capped;
    }
    for (int offset = warpSize / 2; offset > 0; offset >>= 1)
        max_val = fmaxf(max_val, __shfl_down_sync(0xffffffff, max_val, offset));
    ...
    if (threadIdx.x == 0) {
        float log_sum_exp = max_val + logf(sum_exp);
        float capped_target = pg_softcap_asym(pg_bf16_to_f32(row[target]), softcap, softcap_neg);
        losses[t] = log_sum_exp - capped_target;
    }
}
...
extern "C" __global__ void cross_entropy_backward(
    ...
    for (int i = threadIdx.x; i < vocab_size; i += blockDim.x) {
        ...
        float d_softcap = pg_softcap_asym_deriv(row[i], softcap, softcap_neg);
...
extern "C" __global__ void cross_entropy_backward_bf16_logits(
    ...
    for (int i = threadIdx.x; i < vocab_size; i += blockDim.x) {
        ...
        float capped = pg_softcap_asym(logit, softcap, softcap_neg);
        float prob = expf(capped - max_val) / sum_exp;
        float one_hot = (i == target) ? 1.0f : 0.0f;
        float d_softcap = pg_softcap_asym_deriv(logit, softcap, softcap_neg);
        float grad = grad_loss * (prob - one_hot) * d_softcap;
...
extern "C" __global__ void output_ce_tile_stats_update(
    ...
    if (threadIdx.x == 0) {
        ...
        if (target >= vocab_start && target < vocab_start + tile_vocab) {
            ...
            target_logit[t] = pg_softcap_asym(row[local], softcap, softcap_neg);
        }
    }
}
...
extern "C" __global__ void output_ce_tile_grad_bf16(
    ...
    float grad = grad_loss * (prob - one_hot) * pg_softcap_asym_deriv(logit, softcap, softcap_neg);
...
// out = grad + hyper[momentum] * momentum. This keeps captured Muon graphs
// correct across steps without recapturing when momentum changes.
extern "C" __global__ void nesterov_update_hyper(
    const float* __restrict__ grad,
    const float* __restrict__ momentum_buf,
    float* __restrict__ out,
    const float* __restrict__ hyper,
    int n
) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    out[idx] = grad[idx] + hyper[1] * momentum_buf[idx];
}

// x = hyper[momentum] * x + y.
extern "C" __global__ void scale_add_inplace_hyper_momentum(
    float* __restrict__ x,
    const float* __restrict__ y,
    const float* __restrict__ hyper,
    int n
    ...
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if (idx >= n) return;
    x[idx] = hyper[1] * x[idx] + y[idx];
}

// x = hyper[momentum] * x + clip(sum_sq) * y.
extern "C" __global__ void scale_add_inplace_global_norm_hyper_momentum(
    float* __restrict__ x,
    const float* __restrict__ y,
    const float* __restrict__ sum_sq,
    const float* __restrict__ hyper,
    ...
    float norm = sqrtf(sum_sq[0]);
    float clip = (norm > max_norm) ? (max_norm / (norm + 1e-12f)) : 1.0f;
    x[idx] = hyper[1] * x[idx] + clip * y[idx];
}
...
extern "C" __global__ void sparse_attn_gate_xsa_forward_bf16_bhsd_warpheads(
    const unsigned short* __restrict__ y_bhsd,
    const unsigned short* __restrict__ v_bhsd,
    const float* __restrict__ gate_input,
    const float* __restrict__ weight,
    ...
    int s = token - b * seq_len;
    int hkv = head / (num_heads / num_kv_heads);
    int out_off = (token * num_heads + head) * head_dim;
    int y_bf16_off = ((b * num_heads + head) * seq_len + s) * head_dim;
    int v_bf16_off = ((b * num_kv_heads + hkv) * seq_len + s) * head_dim;
    ...
    float score = 0.0f;
    if (lane < gate_width) {
        score = weight[head * gate_width + lane] * gate_input[token * model_dim + lane];
    }
    for (int offset = 16; offset > 0; offset >>= 1) {
    ...
    float coeff = __shfl_sync(0xffffffff, y_dot_v, 0)
        / (__shfl_sync(0xffffffff, v_norm_sq, 0) + 1e-8f);
    float gate = 1.0f / (1.0f + expf(-gate_scale * __shfl_sync(0xffffffff, score, 0)));
    if (lane == 0) {
        gate_values[token * num_heads + head] = gate;
    }

    for (int i = lane; i < head_dim; i += 32) {
        float yv = pg_bf16_to_f32(y_bhsd[y_bf16_off + i]);
        float vv = pg_bf16_to_f32(v_bhsd[v_bf16_off + i]);
        float value = (yv - coeff * vv) * gate;
        out[out_off + i] = value;
        out_bf16[out_off + i] = pg_f32_to_bf16(value);
    }
}
...
extern "C" __global__ void sparse_attn_gate_xsa_backward_bf16_bhsd_do_bf16(
    const unsigned short* __restrict__ y_bhsd,
    const unsigned short* __restrict__ v_bhsd,
    const float* __restrict__ gate_input,
    const float* __restrict__ gate_values,
...
extern "C" __global__ void sparse_attn_gate_xsa_backward_bf16_bhsd_warpheads_do_bf16(
    const unsigned short* __restrict__ y_bhsd,
    const unsigned short* __restrict__ v_bhsd,
    const float* __restrict__ gate_input,
    const float* __restrict__ gate_values,
...
extern "C" __global__ void sparse_attn_gate_xsa_backward_bf16_bhsd_grouped_kv_do_bf16(
    ...
    int tokens = batch * seq_len;
    if (kv_block >= tokens * num_kv_heads) return;
    if (num_kv_heads <= 0 || num_heads % num_kv_heads != 0) return;
    int heads_per_kv = num_heads / num_kv_heads;
    if (heads_per_kv <= 0 || heads_per_kv > 8 || head_dim > 128 || gate_width > 128) return;

    int token = kv_block / num_kv_heads;
    int hkv = kv_block - token * num_kv_heads;
    int b = token / seq_len;
    int s = token - b * seq_len;
    int lane = threadIdx.x & 31;
    int q_local = threadIdx.x >> 5;
    if (q_local >= heads_per_kv) return;

    int h = hkv * heads_per_kv + q_local;
    int head_idx = token * num_heads + h;
    int y_f32_off = head_idx * head_dim;
    int v_f32_off = (token * num_kv_heads + hkv) * head_dim;
    int y_bf16_off = ((b * num_heads + h) * seq_len + s) * head_dim;
    ...
    __shared__ float gv_partial[8][128];
    ...
    for (int i = lane; i < head_dim; i += 32) {
        ...
        float gxsa = grad_out[y_f32_off + i] * gate;
        grad_y_bhsd_bf16[y_bf16_off + i] = pg_f32_to_bf16(gxsa - go_coeff * vv);
        gv_partial[q_local][i] = -coeff * gxsa - go_coeff * yv
            + (2.0f * y_dot_v_s * go_dot_v_s / v_norm_sq2) * vv;
    }
    __syncthreads();
    ...
    if (q_local == 0) {
        for (int i = lane; i < head_dim; i += 32) {
            ...
            #pragma unroll
            for (int q = 0; q < 8; ++q) {
                if (q < heads_per_kv) gv += gv_partial[q][i];
            }
            grad_v[v_f32_off + i] = gv;
        }
    }
...
extern "C" __global__ void sparse_attn_gate_xsa_backward_bf16_bhsd_grouped_kv(
    const unsigned short* __restrict__ y_bhsd,
    const unsigned short* __restrict__ v_bhsd,
    const float* __restrict__ gate_input,
    const float* __restrict__ gate_values,
...
extern "C" __global__ void sparse_attn_gate_xsa_backward_bf16_bhsd_token_grouped_kv(
    const unsigned short* __restrict__ y_bhsd,
    const unsigned short* __restrict__ v_bhsd,
    const float* __restrict__ gate_input,
    const float* __restrict__ gate_values,
    ...
    if (num_kv_heads <= 0 || num_heads % num_kv_heads != 0) return;
    int heads_per_kv = num_heads / num_kv_heads;
    if (heads_per_kv <= 0 || heads_per_kv > 8 || num_heads > 32 || head_dim > 128 || gate_width > 128) return;
    ...
    __shared__ float gv_partial[32][128];
    ...
    for (int i = lane; i < head_dim; i += 32) {
        ...
        float gxsa = grad_out[y_f32_off + i] * gate;
        grad_y[y_f32_off + i] = gxsa - go_coeff * vv;
        gv_partial[h][i] = -coeff * gxsa - go_coeff * yv
            + (2.0f * y_dot_v_s * go_dot_v_s / v_norm_sq2) * vv;
    }
    __syncthreads();

    if (h < num_kv_heads) {
        int write_v_off = (token * num_kv_heads + h) * head_dim;
        int first_q_head = h * heads_per_kv;
        for (int i = lane; i < head_dim; i += 32) {
            float gv = 0.0f;
            #pragma unroll
            for (int q = 0; q < 8; ++q) {
                if (q < heads_per_kv) gv += gv_partial[first_q_head + q][i];
            }
            grad_v[write_v_off + i] = gv;
        }
    }
...
fn residual_scale_tiled_chunk_reduce_enabled() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        matches!(
...
impl GpuKernels {
    ...
    pub fn new(ctx: Arc<CudaContext>, stream: Arc<CudaStream>) -> PgResult<Self> {
        ...
        Ok(Self {
            ...
            rms_norm_bwd_residual_mix_go_bf16_add_no_mix_grad: module
                .load_function(
                    "rms_norm_backward_accum_residual_mix_backward_go_bf16_add_no_mix_grad",
                )
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            rms_norm_bwd_residual_mix_go_bf16_add_compact_no_mix_grad: module
                .load_function(
                    "rms_norm_backward_accum_residual_mix_backward_go_bf16_add_compact_no_mix_grad",
                )
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_mix_grad_reduce_from_stats_go_bf16_add: module
                .load_function("residual_mix_grad_reduce_from_stats_go_bf16_add")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_mix_grad_reduce_from_stats_go_bf16_add_compact: module
                .load_function("residual_mix_grad_reduce_from_stats_go_bf16_add_compact")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            rms_norm_bwd_residual_mix_go_bf16_chunked: module
                .load_function("rms_norm_backward_accum_residual_mix_backward_go_bf16_chunked")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            rms_norm_bwd_residual_mix_go_bf16_add_chunked: module
                .load_function("rms_norm_backward_accum_residual_mix_backward_go_bf16_add_chunked")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            rms_norm_bwd_residual_mix_go_bf16_add_chunked_compact: module
                .load_function(
                    "rms_norm_backward_accum_residual_mix_backward_go_bf16_add_chunked_compact",
                )
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            leaky_relu_sq_fwd_bf16: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_fwd_x_bf16_only_vec4: module
                .load_function("leaky_relu_sq_forward_x_bf16_only_vec4")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            leaky_relu_sq_bwd_x_bf16_only: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_bf16_only_fast: module
                .load_function("leaky_relu_sq_backward_x_bf16_only_fast")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_bf16_only_vec2: module
                .load_function("leaky_relu_sq_backward_x_bf16_only_vec2")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_bf16_only_vec4_fast: module
                .load_function("leaky_relu_sq_backward_x_bf16_only_vec4_fast")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_go_bf16_only: module
                .load_function("leaky_relu_sq_backward_x_go_bf16_only")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_go_bf16_only_fast: module
                .load_function("leaky_relu_sq_backward_x_go_bf16_only_fast")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            leaky_relu_sq_bwd_x_go_bf16_only_vec4_fast: module
                .load_function("leaky_relu_sq_backward_x_go_bf16_only_vec4_fast")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_mix: module
            ...
            residual_add_scale_bwd: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_assign: module
                .load_function("residual_add_scale_backward_assign")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            residual_add_scale_bwd_bf16_only_no_atomic: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_bf16_only_no_atomic_add: module
                .load_function("residual_add_scale_backward_bf16_only_no_atomic_add")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            residual_add_scale_grad_scale_reduce_bf16: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_from_bf16_only_chunked_stage1: module
                .load_function("residual_add_scale_backward_from_bf16_only_chunked_stage1")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_bf16_only_chunked_stage1: module
                .load_function("residual_add_scale_backward_bf16_only_chunked_stage1")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_from_bf16_only_chunked_stage1_add: module
                .load_function("residual_add_scale_backward_from_bf16_only_chunked_stage1_add")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_from_bf16_only_chunked_tiled_stage1: module
                .load_function("residual_add_scale_backward_from_bf16_only_chunked_tiled_stage1")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_bf16_only_chunked_tiled_stage1: module
                .load_function("residual_add_scale_backward_bf16_only_chunked_tiled_stage1")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            residual_add_scale_bwd_from_bf16_only_chunked_tiled_stage1_add: module
                .load_function(
            ...
            shifted_u16_to_u32: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            shifted_u16_ring_to_u32: module
                .load_function("shifted_u16_ring_to_u32")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            q_gain_rope_qk_norm_bwd_chunked_go_bf16_out_bf16: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            q_gain_rope_qk_norm_bwd_chunked_go_bf16_pack_q: module
                .load_function("q_gain_rope_qk_norm_backward_chunked_go_bf16_pack_q")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            qkv_rope_qk_norm_bwd_chunked_go_bf16_pack: module
                .load_function("qkv_rope_qk_norm_backward_chunked_go_bf16_pack")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            rope_qk_norm_bwd_go_bf16_out_bf16: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            rope_qk_norm_bwd_go_bf16_pack_kv: module
                .load_function("rope_qk_norm_backward_go_bf16_pack_kv")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            dot_accumulate: module
                .load_function("dot_accumulate")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            scale_square_accumulate: module
                .load_function("scale_square_accumulate")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            bf16_to_f32_scale_square_accumulate: module
                .load_function("bf16_to_f32_scale_square_accumulate")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            nesterov_update_hyper: module
                .load_function("nesterov_update_hyper")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            scale_add_inplace_hyper_momentum: module
                .load_function("scale_add_inplace_hyper_momentum")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            scale_add_inplace_global_norm_hyper_momentum: module
                .load_function("scale_add_inplace_global_norm_hyper_momentum")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            sparse_attn_gate_xsa_fwd_bf16_bhsd: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            sparse_attn_gate_xsa_fwd_bf16_bhsd_warpheads: module
                .load_function("sparse_attn_gate_xsa_forward_bf16_bhsd_warpheads")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            sparse_attn_gate_xsa_bwd_bf16_bhsd_do_bf16: module
                .load_function("sparse_attn_gate_xsa_backward_bf16_bhsd_do_bf16")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            sparse_attn_gate_xsa_bwd_bf16_bhsd_token_grouped_kv: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            sparse_attn_gate_xsa_bwd_bf16_bhsd_grouped_kv_do_bf16: module
                .load_function("sparse_attn_gate_xsa_backward_bf16_bhsd_grouped_kv_do_bf16")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            fill_u16: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            synthetic_record_batch_u32: module
                .load_function("synthetic_record_batch_u32")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            unpack_qkv_output: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            unpack_qkv_output_save_qk: module
                .load_function("unpack_qkv_output_save_qk")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            unpack_qkv_rope_gain_prepack_fwd: module
                .load_function("unpack_qkv_rope_gain_prepack_forward")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            unpack_qkv_rope_gain_prepack_fwd_bf16_only: module
                .load_function("unpack_qkv_rope_gain_prepack_forward_bf16_only")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            ...
            decay_sgd_step: module
                ...
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            decay_sgd_step_bf16_shadow: module
                .load_function("decay_sgd_step_bf16_shadow")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            decay_sgd_step_hyper: module
                .load_function("decay_sgd_step_hyper")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            decay_sgd_step_bf16_shadow_hyper: module
                .load_function("decay_sgd_step_bf16_shadow_hyper")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            adamw_step: module
                .load_function("adamw_step")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
            adamw_step_bf16_shadow: module
                .load_function("adamw_step_bf16_shadow")
                .map_err(|e| PgError::InvalidOp(format!("Failed to load kernel {}", e)))?,
        })
    }
    ...
    pub fn synthetic_record_batch_u32(
        &self,
        input_ids: CudaPtr,
        targets: CudaPtr,
        step: u64,
        ...
        world_size: u32,
        seq_len: u32,
        local_microbatches: u32,
        vocab_size: u32,
    ) -> PgResult<()> {
        let n = seq_len.saturating_mul(local_microbatches);
        if n == 0 {
            return Ok(());
        }
        let block = 256u32;
        ...
        unsafe {
            self.stream
                .launch_builder(&self.synthetic_record_batch_u32)
                .arg(&input_ids)
                .arg(&targets)
                .arg(&step)
                .arg(&global_batch_tokens)
                .arg(&rank)
                .arg(&world_size)
                .arg(&seq_len)
                .arg(&local_microbatches)
                .arg(&vocab_size)
                .launch(cudarc::driver::LaunchConfig {
                    grid_dim: (grid, 1, 1).into(),
                    block_dim: (block, 1, 1).into(),
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("synthetic_record_batch_u32 failed: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn unpack_qkv_output_save_qk(
        &self,
        combined: CudaPtr,
        q: CudaPtr,
        k: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.unpack_qkv_output_save_qk)
                .arg(&combined)
                .arg(&q)
                .arg(&k)
                .arg(&v)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("unpack_qkv_output_save_qk failed: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn unpack_qkv_rope_gain_prepack_fwd(
        &self,
        combined: CudaPtr,
        q: CudaPtr,
        k: CudaPtr,
        ...
        let block = 128u32;
        let heads = num_heads.max(num_kv_heads);
        unsafe {
            self.stream
                .launch_builder(&self.unpack_qkv_rope_gain_prepack_fwd)
                .arg(&combined)
                .arg(&q)
                .arg(&k)
                .arg(&v)
                ...
                .arg(&eps)
                .launch(cudarc::driver::LaunchConfig {
                    grid_dim: (tokens, heads, 1).into(),
                    block_dim: (block, 1, 1).into(),
                    shared_mem_bytes: 0,
                })
                .map_err(|e| {
                    PgError::InvalidOp(format!("unpack_qkv_rope_gain_prepack_fwd failed: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn unpack_qkv_rope_gain_prepack_fwd_bf16_only(
        &self,
        combined: CudaPtr,
        q_bhsd_bf16: CudaPtr,
        k_bhsd_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.unpack_qkv_rope_gain_prepack_fwd_bf16_only)
                .arg(&combined)
                .arg(&q_bhsd_bf16)
                .arg(&k_bhsd_bf16)
                .arg(&v_bhsd_bf16)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "unpack_qkv_rope_gain_prepack_fwd_bf16_only failed: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_compact(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        ...
        if extra_width == 0 || extra_width > dim {
            return Err(PgError::InvalidOp(format!(
                "compact residual-mix extra width {extra_width} must be in 1..={dim}"
            )));
        }
        let n = num_rows * dim;
        let block = 32u32.max(256u32.min(dim.next_power_of_two()));
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_compact)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_no_mix_grad(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_no_mix_grad)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra)
                .arg(&residual_x)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "rms_norm_bwd_residual_mix_go_bf16_add_no_mix_grad launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_compact_no_mix_grad(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra_compact: CudaPtr,
        ...
        if extra_width == 0 || extra_width > dim {
            return Err(PgError::InvalidOp(format!(
                "compact no-mix residual-mix extra width {extra_width} must be in 1..={dim}"
            )));
        }
        let n = num_rows * dim;
        let block = 32u32.max(256u32.min(dim.next_power_of_two()));
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_compact_no_mix_grad)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra_compact)
                .arg(&residual_x)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "rms_norm_bwd_residual_mix_go_bf16_add_compact_no_mix_grad launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_split_reduce(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra: CudaPtr,
        ...
        let n = num_rows * dim;
        let block = 32u32.max(256u32.min(dim.next_power_of_two()));
        let chunks = num_rows.div_ceil(chunk_rows.max(1));
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_no_mix_grad)
                .arg(&x_norm)
            ...
            self.stream
                .launch_builder(&self.residual_mix_grad_reduce_from_stats_go_bf16_add)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra)
                .arg(&residual_x)
                ...
                .arg(&beta)
                .launch(cudarc::driver::LaunchConfig {
                    grid_dim: (2 * dim, chunks, 1).into(),
                    block_dim: (256, 1, 1).into(),
                    shared_mem_bytes: 0,
                })
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_mix_grad_reduce_from_stats_go_bf16_add launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_compact_split_reduce(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra_compact: CudaPtr,
        ...
        if extra_width == 0 || extra_width > dim {
            return Err(PgError::InvalidOp(format!(
                "compact split-reduce residual-mix extra width {extra_width} must be in 1..={dim}"
            )));
        }
        let n = num_rows * dim;
        let block = 32u32.max(256u32.min(dim.next_power_of_two()));
        ...
        unsafe {
            ...
            self.stream
                .launch_builder(&self.residual_mix_grad_reduce_from_stats_go_bf16_add_compact)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra_compact)
                .arg(&residual_x)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_mix_grad_reduce_from_stats_go_bf16_add_compact launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_chunked(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        residual_x: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_chunked)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&residual_x)
                .arg(&residual_x0)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "rms_norm_bwd_residual_mix_go_bf16_chunked launch: {:?}",
                        e
                    ))
                })?;
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_chunked(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_chunked)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra)
                .arg(&residual_x)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "rms_norm_bwd_residual_mix_go_bf16_add_chunked launch: {:?}",
                        e
                    ))
                })?;
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rms_norm_backward_accum_residual_mix_bwd_go_bf16_add_chunked_compact(
        &self,
        x_norm: CudaPtr,
        grad_norm_bf16: CudaPtr,
        grad_norm_extra_compact: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.rms_norm_bwd_residual_mix_go_bf16_add_chunked_compact)
                .arg(&x_norm)
                .arg(&grad_norm_bf16)
                .arg(&grad_norm_extra_compact)
                .arg(&residual_x)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "rms_norm_bwd_residual_mix_go_bf16_add_chunked_compact launch: {:?}",
                        e
                    ))
                })?;
            ...
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_mix_grad_reduce_chunks compact launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn decay_sgd_step_bf16_shadow(
        &self,
        param: CudaPtr,
        param_bf16: CudaPtr,
        grad: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.decay_sgd_step_bf16_shadow)
                .arg(&param)
                .arg(&param_bf16)
                .arg(&grad)
                .arg(&lr)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("decay_sgd_step_bf16_shadow failed: {:?}", e))
                })?;
        }
        Ok(())
    }

    pub fn decay_sgd_step_hyper(
        &self,
        param: CudaPtr,
        grad: CudaPtr,
        hyper: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.decay_sgd_step_hyper)
                .arg(&param)
                .arg(&grad)
                .arg(&hyper)
                .arg(&(n as i32))
                ...
                .map_err(|e| PgError::InvalidOp(format!("decay_sgd_step_hyper failed: {:?}", e)))?;
        }
        Ok(())
    }

    pub fn decay_sgd_step_bf16_shadow_hyper(
        &self,
        param: CudaPtr,
        param_bf16: CudaPtr,
        grad: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.decay_sgd_step_bf16_shadow_hyper)
                .arg(&param)
                .arg(&param_bf16)
                .arg(&grad)
                .arg(&hyper)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("decay_sgd_step_bf16_shadow_hyper failed: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    pub fn adamw_step_bf16_shadow(
        ...
        unsafe {
            self.stream
                .launch_builder(&self.adamw_step_bf16_shadow)
                .arg(&param)
                .arg(&param_bf16)
                .arg(&grad)
                .arg(&m)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("adamw_step_bf16_shadow failed: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    pub fn leaky_relu_sq_forward_x_bf16_only(
        ...
        let block = 256u32;
        let groups = (n + 3) / 4;
        let grid = (groups + block - 1) / block;
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_fwd_x_bf16_only_vec4)
                .arg(&x_bf16)
                .arg(&y_bf16)
                .arg(&(n as i32))
                .launch(cudarc::driver::LaunchConfig {
    ...
    pub fn leaky_relu_sq_backward_x_bf16_only_fast(
        &self,
        x_bf16: CudaPtr,
        grad_output: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_bf16_only_fast)
                .arg(&x_bf16)
                .arg(&grad_output)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "leaky_relu_sq_bwd_x_bf16_only_fast launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn leaky_relu_sq_backward_x_bf16_only_vec2(
        &self,
        x_bf16: CudaPtr,
        grad_output: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        let block = 256u32;
        let pairs = (n + 1) / 2;
        let grid = (pairs + block - 1) / block;
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_bf16_only_vec2)
                .arg(&x_bf16)
                .arg(&grad_output)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "leaky_relu_sq_bwd_x_bf16_only_vec2 launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn leaky_relu_sq_backward_x_bf16_only_vec4_fast(
        &self,
        x_bf16: CudaPtr,
        grad_output: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_bf16_only_vec4_fast)
                .arg(&x_bf16)
                .arg(&grad_output)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "leaky_relu_sq_bwd_x_bf16_only_vec4_fast launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn leaky_relu_sq_backward_x_go_bf16_only(
        &self,
        x_bf16: CudaPtr,
        grad_output_bf16: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_go_bf16_only)
                .arg(&x_bf16)
                .arg(&grad_output_bf16)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("leaky_relu_sq_bwd_x_go_bf16_only launch: {:?}", e))
                })?;
        }
        Ok(())
    }

    pub fn leaky_relu_sq_backward_x_go_bf16_only_fast(
        &self,
        x_bf16: CudaPtr,
        grad_output_bf16: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_go_bf16_only_fast)
                .arg(&x_bf16)
                .arg(&grad_output_bf16)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "leaky_relu_sq_bwd_x_go_bf16_only_fast launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn leaky_relu_sq_backward_x_go_bf16_only_vec4_fast(
        &self,
        x_bf16: CudaPtr,
        grad_output_bf16: CudaPtr,
        grad_input_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.leaky_relu_sq_bwd_x_go_bf16_only_vec4_fast)
                .arg(&x_bf16)
                .arg(&grad_output_bf16)
                .arg(&grad_input_bf16)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "leaky_relu_sq_bwd_x_go_bf16_only_vec4_fast launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn residual_add_scale_bwd_assign(
        &self,
        proj: CudaPtr,
        grad_output: CudaPtr,
        scale: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.residual_add_scale_bwd_assign)
                .arg(&grad_output)
                .arg(&scale)
                .arg(&grad_x_in)
                .arg(&grad_proj)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("residual_add_scale_bwd_assign launch: {:?}", e))
                })?;
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_assign reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn residual_add_scale_bwd_from_bf16_only_add(
        &self,
        proj_bf16: CudaPtr,
        grad_output: CudaPtr,
        add_input: CudaPtr,
        ...
        if !residual_scale_reduce_enabled() {
            return Err(PgError::InvalidOp(
                "residual_add_scale_bwd_from_bf16_only_add requires residual-scale reduce mode"
                    .into(),
            ));
        }
        let block = 256u32;
        ...
        unsafe {
            self.stream
                .launch_builder(&self.residual_add_scale_bwd_bf16_only_no_atomic_add)
                .arg(&grad_output)
                .arg(&add_input)
                .arg(&scale)
                .arg(&grad_x_in)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_from_bf16_only_add no_atomic_add launch: {:?}",
                        e
                    ))
                })?;
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_from_bf16_only_add reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn residual_add_scale_bwd_from_bf16_only_chunked(
        ...
        if dim > 1024 {
            return Err(PgError::InvalidOp(format!(
                "chunked residual add-scale reduce supports dim <= 1024, got {dim}"
            )));
        }
        let rows = n / dim;
        let chunks = rows.div_ceil(rows_per_chunk.max(1));
        let block = 256u32;
        unsafe {
            if residual_scale_tiled_chunk_reduce_enabled() {
                let dim_tile = 16u32;
                let dim_tiles = dim.div_ceil(dim_tile);
                self.stream
                    .launch_builder(
                        &self.residual_add_scale_bwd_from_bf16_only_chunked_tiled_stage1,
                    )
                    .arg(&proj_bf16)
                    .arg(&grad_output)
                    .arg(&scale)
                    ...
                    .arg(&(dim_tile as i32))
                    .launch(cudarc::driver::LaunchConfig {
                        grid_dim: (chunks, dim_tiles, 1).into(),
                        block_dim: (block, 1, 1).into(),
                        shared_mem_bytes: block * std::mem::size_of::<f32>() as u32,
                    })
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_from_bf16_only_chunked_tiled stage1 launch: {:?}",
                            e
                        ))
                    })?;
            } else {
                self.stream
                    .launch_builder(&self.residual_add_scale_bwd_from_bf16_only_chunked_stage1)
                    .arg(&proj_bf16)
                    .arg(&grad_output)
                    .arg(&scale)
                    .arg(&grad_x_in)
                    ...
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_from_bf16_only_chunked stage1 launch: {:?}",
                            e
                        ))
                    })?;
            }
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_from_bf16_only_chunked reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn residual_add_scale_bwd_from_bf16_only_chunked_add(
        &self,
        proj_bf16: CudaPtr,
        grad_output: CudaPtr,
        add_input: CudaPtr,
        ...
        unsafe {
            if residual_scale_tiled_chunk_reduce_enabled() {
                ...
                self.stream
                    .launch_builder(
                        &self.residual_add_scale_bwd_from_bf16_only_chunked_tiled_stage1_add,
                    )
                    .arg(&proj_bf16)
                    .arg(&grad_output)
                    .arg(&add_input)
                    ...
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_from_bf16_only_chunked_add tiled stage1 launch: {:?}",
                            e
                        ))
                    })?;
            } else {
                self.stream
                    .launch_builder(&self.residual_add_scale_bwd_from_bf16_only_chunked_stage1_add)
                    .arg(&proj_bf16)
                    .arg(&grad_output)
                    .arg(&add_input)
                    .arg(&scale)
                    ...
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_from_bf16_only_chunked_add stage1 launch: {:?}",
                            e
                        ))
                    })?;
            }
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_from_bf16_only_chunked_add reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn residual_add_scale_bwd_bf16_only_chunked(
        &self,
        proj: CudaPtr,
        grad_output: CudaPtr,
        scale: CudaPtr,
        ...
        unsafe {
            if residual_scale_tiled_chunk_reduce_enabled() {
                ...
                let dim_tiles = dim.div_ceil(dim_tile);
                self.stream
                    .launch_builder(&self.residual_add_scale_bwd_bf16_only_chunked_tiled_stage1)
                    .arg(&proj)
                    .arg(&grad_output)
                    .arg(&scale)
                    .arg(&grad_x_in)
                    ...
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_bf16_only_chunked_tiled stage1 launch: {:?}",
                            e
                        ))
                    })?;
            } else {
                self.stream
                    .launch_builder(&self.residual_add_scale_bwd_bf16_only_chunked_stage1)
                    .arg(&proj)
                    .arg(&grad_output)
                    .arg(&scale)
                    .arg(&grad_x_in)
                    ...
                    .map_err(|e| {
                        PgError::InvalidOp(format!(
                            "residual_add_scale_bwd_bf16_only_chunked stage1 launch: {:?}",
                            e
                        ))
                    })?;
            }
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "residual_add_scale_bwd_bf16_only_chunked reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    /// Expand one compact shifted span from a device-resident token ring.
    pub fn shifted_u16_ring_to_u32(
        &self,
        ring: CudaPtr,
        input: CudaPtr,
        target: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.shifted_u16_ring_to_u32)
                .arg(&ring)
                .arg(&input)
                .arg(&target)
                .arg(&(tokens as i32))
                .arg(&(ring_offset as i32))
                .launch(cudarc::driver::LaunchConfig {
                    grid_dim: (grid, 1, 1).into(),
                    block_dim: (block, 1, 1).into(),
                    shared_mem_bytes: 0,
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("shifted_u16_ring_to_u32 launch: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn embedding_bigram_project_merge_fwd(
        &self,
        ids: CudaPtr,
        tok_emb: CudaPtr,
        ...
        if bigram_vocab < 2 {
            return Err(PgError::InvalidOp(
                "embedding_bigram_project_merge requires at least 2 bigram buckets".into(),
            ));
        }
        let n = tokens
            .checked_mul(model_dim)
            .ok_or_else(|| PgError::InvalidOp("embedding_bigram_project_merge overflow".into()))?;
        let block = 256u32;
        let grid = (n + block - 1) / block;
        unsafe {
            self.stream
                .launch_builder(&self.embedding_bigram_project_merge)
                .arg(&ids)
                .arg(&tok_emb)
                .arg(&bigram_embed)
                .arg(&bigram_proj)
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn q_gain_rope_qk_norm_bwd_chunked_go_bf16_pack_q(
        &self,
        q_pre_norm: CudaPtr,
        q_post_rope: CudaPtr,
        grad_q_scaled_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.q_gain_rope_qk_norm_bwd_chunked_go_bf16_pack_q)
                .arg(&q_pre_norm)
                .arg(&q_post_rope)
                .arg(&grad_q_scaled_bf16)
                .arg(&gain)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "q_gain_rope_qk_norm_bwd_chunked_go_bf16_pack_q launch: {:?}",
                        e
                    ))
                })?;
            self.stream
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn qkv_rope_qk_norm_bwd_chunked_go_bf16_pack(
        &self,
        q_pre_norm: CudaPtr,
        q_post_rope: CudaPtr,
        grad_q_scaled_bf16: CudaPtr,
        ...
        let warps = num_heads + num_kv_heads;
        if warps == 0 || warps > 32 {
            return Err(PgError::InvalidOp(format!(
                "combined qkv rope tail requires 1..=32 total warps, got {warps}"
            )));
        }
        let block = 32;
        let grid = tokens
            .checked_mul(warps)
            .ok_or_else(|| PgError::InvalidOp("combined qkv rope tail grid overflow".into()))?;
        unsafe {
            self.stream
                .launch_builder(&self.qkv_rope_qk_norm_bwd_chunked_go_bf16_pack)
                .arg(&q_pre_norm)
                .arg(&q_post_rope)
                .arg(&grad_q_scaled_bf16)
                .arg(&k_pre_norm)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "qkv_rope_qk_norm_bwd_chunked_go_bf16_pack launch: {:?}",
                        e
                    ))
                })?;
            self.stream
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn rope_qk_norm_bwd_go_bf16_pack_kv(
        &self,
        k_pre_norm: CudaPtr,
        grad_k_attn_bf16: CudaPtr,
        grad_v_bf16: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.rope_qk_norm_bwd_go_bf16_pack_kv)
                .arg(&k_pre_norm)
                .arg(&grad_k_attn_bf16)
                .arg(&grad_v_bf16)
                .arg(&grad_v_xsa)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("rope_qk_norm_bwd_go_bf16_pack_kv launch: {:?}", e))
                })?;
        }
        Ok(())
    }
    ...
    /// x *= scale; out[0] += alpha * sum(x * x) after scaling.
    pub fn scale_square_accumulate(
        &self,
        x: CudaPtr,
        out: CudaPtr,
        scale: f32,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.scale_square_accumulate)
                .arg(&x)
                .arg(&out)
                .arg(&scale)
                .arg(&alpha)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("scale_square_accumulate launch: {:?}", e))
                })?;
        }
        Ok(())
    }

    /// dst = bf16_to_f32(src) * scale; out[0] += alpha * sum(dst * dst).
    pub fn bf16_to_f32_scale_square_accumulate(
        &self,
        src: CudaPtr,
        dst: CudaPtr,
        out: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.bf16_to_f32_scale_square_accumulate)
                .arg(&src)
                .arg(&dst)
                .arg(&out)
                .arg(&scale)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "bf16_to_f32_scale_square_accumulate launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    pub fn nesterov_update_hyper_fwd(
        &self,
        grad: CudaPtr,
        momentum: CudaPtr,
        out: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.nesterov_update_hyper)
                .arg(&grad)
                .arg(&momentum)
                .arg(&out)
                .arg(&hyper)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("nesterov_update_hyper launch: {:?}", e))
                })?;
        }
        Ok(())
    }

    pub fn scale_add_inplace_hyper_momentum_fwd(
        &self,
        x: CudaPtr,
        y: CudaPtr,
        hyper: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.scale_add_inplace_hyper_momentum)
                .arg(&x)
                .arg(&y)
                .arg(&hyper)
                .arg(&(n as i32))
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!("scale_add_inplace_hyper_momentum launch: {:?}", e))
                })?;
        }
        Ok(())
    }

    pub fn scale_add_inplace_global_norm_hyper_momentum_fwd(
        &self,
        x: CudaPtr,
        y: CudaPtr,
        sum_sq: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.scale_add_inplace_global_norm_hyper_momentum)
                .arg(&x)
                .arg(&y)
                .arg(&sum_sq)
                .arg(&hyper)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "scale_add_inplace_global_norm_hyper_momentum launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_fwd_bf16_bhsd_warpheads(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        if gate_width == 0 || gate_width > model_dim || gate_width > 32 {
            return self.sparse_attn_gate_xsa_fwd_bf16_bhsd(
                y_bhsd,
                v_bhsd,
                gate_input,
                weight,
        ...
        let tokens = batch
            .checked_mul(seq_len)
            .ok_or_else(|| PgError::InvalidOp("SparseAttnGate+XSA BF16 token overflow".into()))?;
        let block = 32 * num_heads.next_power_of_two().max(1);
        unsafe {
            self.stream
                .launch_builder(&self.sparse_attn_gate_xsa_fwd_bf16_bhsd_warpheads)
                .arg(&y_bhsd)
                .arg(&v_bhsd)
                .arg(&gate_input)
                .arg(&weight)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "sparse_attn_gate_xsa_fwd_bf16_bhsd_warpheads launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_two_pass_compact_grad_input(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        if num_heads == 0 || num_heads * 32 > 1024 {
            return Err(PgError::InvalidOp(format!(
                "warp-head compact SparseAttnGate XSA backward requires 1..=32 heads; got {num_heads}"
            )));
        }
        if num_kv_heads == 0 || num_heads % num_kv_heads != 0 {
            return Err(PgError::InvalidOp(format!(
        ...
        if num_kv_heads > 32 || head_dim > 128 || gate_width > 128 {
            return Err(PgError::InvalidOp(
                "compact SparseAttnGate XSA backward only supports the warp-head BF16 path".into(),
            ));
        }
        let tokens = batch
            .checked_mul(seq_len)
        ...
        unsafe {
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "compact sparse_attn_gate_xsa_backward_bf16_bhsd_warpheads launch: {:?}",
                        e
                    ))
                })?;
            self.stream
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_do_bf16_two_pass(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.sparse_attn_gate_xsa_bwd_bf16_bhsd_do_bf16)
                .arg(&y_bhsd)
                .arg(&v_bhsd)
                .arg(&gate_input)
                .arg(&gate_values_and_grad_score)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "sparse_attn_gate_xsa_backward_bf16_bhsd_do_bf16 launch: {:?}",
                        e
                    ))
                })?;
            self.stream
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_do_bf16_two_pass(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        if num_kv_heads > 32 || head_dim > 128 || gate_width > 128 {
            return self.sparse_attn_gate_xsa_bwd_bf16_bhsd_do_bf16_two_pass(
                y_bhsd,
                v_bhsd,
                gate_input,
                gate_values_and_grad_score,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_do_bf16)
                .arg(&y_bhsd)
                .arg(&v_bhsd)
                .arg(&gate_input)
                .arg(&gate_values_and_grad_score)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "sparse_attn_gate_xsa_backward_bf16_bhsd_warpheads_do_bf16 launch: {:?}",
                        e
                    ))
                })?;
            self.stream
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_do_bf16_two_pass_compact_grad_input(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_grouped_kv_two_pass(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        if num_heads == 0
            ...
            || gate_width > 128
        {
            return self.sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_two_pass(
                y_bhsd,
                v_bhsd,
                gate_input,
                gate_values_and_grad_score,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.sparse_attn_gate_xsa_bwd_bf16_bhsd_token_grouped_kv)
                .arg(&y_bhsd)
                .arg(&v_bhsd)
                .arg(&gate_input)
                .arg(&gate_values_and_grad_score)
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "sparse_attn_gate_xsa_backward_bf16_bhsd_token_grouped_kv launch: {:?}",
                        e
                    ))
                })?;
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "grouped-kv sparse_attn_gate_weight_grad_reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_grouped_kv_two_pass_compact_grad_input(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        {
            return Err(PgError::InvalidOp(
                "compact token-grouped SparseAttnGate XSA backward only supports the warp-head BF16 shape".into(),
            ));
        }
        let tokens = batch
            .checked_mul(seq_len)
        ...
        unsafe {
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "compact sparse_attn_gate_xsa_backward_bf16_bhsd_token_grouped_kv launch: {:?}",
                        e
                    ))
                })?;
            self.stream
                ...
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "compact token-grouped sparse_attn_gate_weight_grad_reduce launch: {:?}",
                        e
                    ))
                })?;
        }
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn sparse_attn_gate_xsa_bwd_bf16_bhsd_grouped_kv_do_bf16_two_pass(
        &self,
        y_bhsd: CudaPtr,
        v_bhsd: CudaPtr,
        gate_input: CudaPtr,
        ...
        if num_kv_heads == 0 || num_heads == 0 || num_heads % num_kv_heads != 0 {
            return Err(PgError::InvalidOp(format!(
                "num_heads ({num_heads}) must be divisible by num_kv_heads ({num_kv_heads})"
            )));
        }
        ...
        if heads_per_kv == 0
            || heads_per_kv > 8
            || heads_per_kv * 32 > 1024
            || head_dim > 128
            || gate_width > 128
        {
            return self.sparse_attn_gate_xsa_bwd_bf16_bhsd_warpheads_do_bf16_two_pass(
                y_bhsd,
                v_bhsd,
                gate_input,
                gate_values_and_grad_score,
        ...
        unsafe {
            self.stream
                .launch_builder(&self.sparse_attn_gate_xsa_bwd_bf16_bhsd_grouped_kv_do_bf16)
                .arg(&y_bhsd)
                .arg(&v_bhsd)
                .arg(&gate_input)
                .arg(&gate_values_and_grad_score)
                ...
                .arg(&gate_scale)
                .launch(cudarc::driver::LaunchConfig {
                    grid_dim: (tokens * num_kv_heads, 1, 1).into(),
                    block_dim: (heads_per_kv * 32, 1, 1).into(),
                    shared_mem_bytes: 0,
                })
                .map_err(|e| {
                    PgError::InvalidOp(format!(
                        "sparse_attn_gate_xsa_backward_bf16_bhsd_grouped_kv_do_bf16 launch: {:?}",
                        e
                    ))
                })?;
            self.stream
